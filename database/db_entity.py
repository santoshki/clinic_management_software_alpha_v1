...
def insert_db_solution(solution_data):
    print("Health solution data received.")
    try:
...
def db_insert_vitals(vitals_data):
    print("Patient vitals_data received")
    try:
        ...
        try:
            cursor.execute('''SELECT count(name) FROM sqlite_master WHERE type='table' AND 
                            name='patients_vitals_data' ''')
            if cursor.fetchone()[0] == 0:
                table_create = """CREATE TABLE patients_vitals_data(BP TEXT, PULSERATE TEXT, OXYGENLEVEL TEXT, TEMPERATURE TEXT, 
                HEIGHT TEXT, WEIGHT TEXT)"""
                cursor.execute(table_create)
                conn.commit()
                print("Table created in SQLlite DB.")

            cursor.execute("INSERT INTO patients_vitals_data(BP, PULSERATE, OXYGENLEVEL, TEMPERATURE, HEIGHT, WEIGHT) "
                           "VALUES (?,?,?,?,?,?)",
                           (vitals_data[0], vitals_data[1], vitals_data[2], vitals_data[3], vitals_data[4], vitals_data[5]))
            conn.commit()
            print("Data inserted in SQL lite db")
